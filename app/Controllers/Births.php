...
class Births extends BaseController
{
		...
		switch ($start_message) 
			{
				case 0:
					...
					$session->set('return_route', 'births/transcribe_births_step2');
					$session->set('return_route_step1', 'births/transcribe_births_step1/0');
					if ( $session->database_backup_performed == 1 )
						{
							$session->set('table_title', 'Births - database backup performed');
						}
					else
						{
							$session->set('table_title', 'Births');
						}
					// set dup fields
		...
		switch ($session->show_view_type) 
			{
				case 'transcribe':
					...
					echo view('linBMD2/transcribe_births');
		...
		switch ($session->show_view_type) 
			{
				case 'transcribe':
					...
					$session->set('registration', $this->request->getPost('registration'));
	...
	public function select_comment($line_index)
	{
		...
		// show comment page															
		echo view('templates/header');
		echo view('linBMD2/transcribe_births_comments');
		echo view('linBMD2/transcribe_births_show_details');
		echo view('templates/footer');	
	}
