...
class Deaths extends BaseController
{
		...
		switch ($start_message) 
			{
				case 0:
					...
					$session->set('return_route', 'deaths/transcribe_deaths_step2');
					$session->set('return_route_step1', 'deaths/transcribe_deaths_step1/0');
					if ( $session->database_backup_performed == 1 )
						{
							$session->set('table_title', 'Deaths - database backup performed');
						}
					else
						{
							$session->set('table_title', 'Deaths');
						}
					// set dup fields
		...
		switch ($session->show_view_type) 
			{
				case 'transcribe':
					...
					echo view('linBMD2/transcribe_deaths');
	...
	public function transcribe_deaths_step2()
	{
		...
		switch ($session->show_view_type) 
			{
				case 'transcribe':
					...
					$session->set('age', $this->request->getPost('age'));
		...
		if ( $session->age == '' )
			{
				$session->set('message_2', 'Age cannot be blank.');
				$session->set('message_class_2', 'alert alert-danger');
				return redirect()->to( base_url('deaths/transcribe_deaths_step1/1') );
			}
		if ( is_numeric($session->age) === false )
			{
				$session->set('message_2', 'Age must be numeric.');
				$session->set('message_class_2', 'alert alert-danger');
				return redirect()->to( base_url('deaths/transcribe_deaths_step1/1') );
			}
		// page blank, valid?
		...
		if ( $session->page_ok == 'N' )
			{
				if ( strlen($session->page) != 3  OR is_numeric($session->page) === false )
					{
						$session->set('show_view_type', 'confirm_page');
						$session->set('message_2', 'Page number is usually 3 digits long. You entered => '.$session->page.'. Please confirm your entry or correct it by selecting No.');
						$session->set('message_class_2', 'alert alert-danger');
						return redirect()->to( base_url('deaths/transcribe_deaths_step1/1') );
					}
			}
		...
		if ( ! $session->transcribe_volumes )
			{
				...
				$session->set('volume', '');
				$session->set('message_2', 'No volume data found for this district => '.$session->district.', '.$session->transcribe_allocation[0]['BMD_year'].', '.$session->transcribe_allocation[0]['BMD_quarter'].'. Please enter volume from scan and confirm.');
				$session->set('message_class_2', 'alert alert-danger');
				return redirect()->to( base_url('deaths/transcribe_deaths_step1/1') );
			}
		...
		$session->set('age', strtoupper($session->age));
		$session->set('district', strtoupper($session->district));
		$session->set('volume', strtoupper($session->volume));
