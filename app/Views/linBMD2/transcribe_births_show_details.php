		...
		<table class="table table-hover">
			<thead class="d-block">
				<tr class="row">
					...
					<th style="padding:5px; text-align:center; width:<?php echo($nw); ?>;" scope="col">Registration</th>
					...
					<?php foreach ($session->transcribe_detail_data as $detail): ?>
						<tr class="row">
							<th class="pl-0" style="padding:5px; text-align:center; width:<?php echo($sw); ?>;" scope="row">
								<a id="delete_line" href="<?php echo(base_url('births/delete_line_step1/'.esc($detail['BMD_index']))) ?>"</a>
								<span><input type="checkbox" </input></span>
							</th>
							<td style="padding:5px; text-align:center; width:<?php echo($sw); ?>;">
								<a id="select_line" href="<?php echo(base_url('births/select_line/'.esc($detail['BMD_index']))) ?>"</a>
							...
							<td style="padding:5px; text-align:center; width:<?php echo($nw); ?>;" ><?php echo esc($detail['BMD_registration'])?></td>
							<td style="padding:5px; text-align:center; width:<?php echo($sw); ?>;" ><?php echo esc($detail['BMD_page'])?></td>
							<td style="padding:5px; text-align:center; width:<?php echo($sw); ?>;">
								<a id="select_line" href="<?php echo(base_url('births/select_comment/'.esc($detail['BMD_index']))) ?>"</a>
							...
							<td class="pr-0" style="padding:5px; text-align:center; width:<?php echo($sw); ?>;">
								<a id="remove_comments" href="<?php echo(base_url('births/remove_comments/'.esc($detail['BMD_index']).'/'.esc($detail['BMD_line_sequence']))) ?>"</a>
